...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const [labour, plant, workItems, variations, delays, pours, quantities, dayworks, photos, sections] =
    await Promise.all([
      ...
      supabase.from('variations').select('description, directed_by, directed_at, vr_ref, estimated_cost, crew, register_seq, variation_number, photo_urls, source_quote, confidence').eq('entry_id', entry.id),
