    ...
    try {
      ...
      for (const item of defects) {
        ...
        if (file) {
          ...
          photoPath = `${base}/defect-${item.key}.${photo.extension}`;
          const { error: upErr } = await supabase.storage.from('entry-photos').upload(photoPath, photo.blob, { contentType: photo.contentType, upsert: false });
          if (upErr) throw new Error(upErr.message);
        }
        const { error: defErr } = await supabase.from('plant_defects').insert({ project_id: projectId, plant_id: plant.id, prestart_id: id, item_key: item.key, item_label: item.label, note: defectNotes[item.key]?.trim() || null, photo_path: photoPath, raised_by: auth.user.id });
      ...
      const { error: doneErr } = await supabase.from('plant_prestarts').update({ signature_path: sigPath }).eq('id', id);
