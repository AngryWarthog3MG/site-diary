...
export default async function PlantPage({ searchParams }: { searchParams: Promise<{ project?: string; kept?: string }> }) {
  ...
  const [{ data: register }, { data: todays }, { data: open }, { data: recent }, { data: onJobRows }] = await Promise.all([
    supabase.from('plant_register').select('id, name, kind, make_model, plant_no, ownership, supplier, active, aliases, inspection_basis, inspection_interval_months, registration_required, registration_no, registration_expires_on').eq('org_id', orgId).order('active', { ascending: false }).order('name'),
