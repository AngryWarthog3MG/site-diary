...
export function dayworksSignoffHtml(
  ...
  // The photographs travel beside the HTML, not inside it (README R84): the
  // page gets a placeholder here and the bytes one at a time at print.
  ...
  const plates = lines.map((l, i) => {
    ...
    return [
      ...
      pics.map((src, k) => {
        const key = `i${n}-${k}`;
        images[key] = src;
        return `<figure><img data-photo="${key}" alt="Item ${n}" /></figure>`;
      }).join(''),
      '</div></div>',
    ].join('');
  }).join('');
