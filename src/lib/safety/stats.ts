...
/**
 * Medical treatment or hospital: the injuries a client's monthly report counts. First aid is
 * counted separately, and an injury recorded with no treatment (or none stated) is its own
 * figure — calling it first aid would print a treatment nobody recorded.
 */
export function classify(i: IncidentFacts): 'mti' | 'fai' | 'injury_untreated' | 'near_miss' | 'hazard' | 'other' {
  if (i.kind === 'injury') return i.treatment === 'medical' || i.treatment === 'hospital' ? 'mti' : i.treatment === 'first_aid' ? 'fai' : 'injury_untreated';
...
export interface InjurySummary {
  mti: number; fai: number; untreated: number; nearMiss: number; hazards: number; other: number; notifiable: number;
  hoursWorked: number;
...
export function injurySummary(incidents: readonly IncidentFacts[], hoursWorked: number): InjurySummary {
  const counts = { mti: 0, fai: 0, untreated: 0, nearMiss: 0, hazards: 0, other: 0 };
  for (const i of incidents) {
    const c = classify(i);
    if (c === 'mti') counts.mti += 1; else if (c === 'fai') counts.fai += 1; else if (c === 'injury_untreated') counts.untreated += 1; else if (c === 'near_miss') counts.nearMiss += 1; else if (c === 'hazard') counts.hazards += 1; else counts.other += 1;
  }
  return {
    ...counts,
 ...
 * Pass every injury the job has ever recorded, not a windowed set — a job whose last injury was
