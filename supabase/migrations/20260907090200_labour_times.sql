-- ============================================================================
-- 20260907090200_labour_times.sql
-- Start, finish and break on labour rows; worked hours become arithmetic.
--
-- Hash compatibility: the three new columns are stripped from the canonical
-- JSON when null (the photos-category pattern), so every entry signed before
-- today keeps verifying, and rows without times hash exactly as they always
-- did. When both times are present the DATABASE computes hours = span - break
-- at apply time — the number on the record follows the clock, not a guess.
-- ============================================================================

alter table public.labour
  add column start_time  time,
  add column finish_time time,
  add column break_mins  integer check (break_mins >= 0 and break_mins <= 480);

comment on column public.labour.break_mins is
  'Unpaid break in minutes — usually 30, sometimes none. Subtracted from the span when hours are computed from start/finish.';
