-- ============================================================================
-- Variations: who did the work.
--
-- The owner asked for a variation to be a description, the people who did
-- the job, and the reference — not who directed it, when, or a guessed value.
-- The value belongs to the variation register (set_variation_details), where
-- it is priced and agreed; "directed by" and "directed at" leave the form.
-- Their columns stay for what was recorded before, and so do their places in
-- the canonical JSON of every signed entry.
--
-- `crew` is new and conditional in the hash, like labour's times: absent when
-- null, so every entry signed before today still verifies.
-- ============================================================================

alter table public.variations add column if not exists crew text[];
    ...
    'variations', (
      select coalesce(jsonb_agg(j order by j::text), '[]'::jsonb)
        from (
          ...
          select ((to_jsonb(t) - 'id' - 'entry_id' - 'created_at')
                  - (case when t.crew is null then 'crew' else '' end)) as j
...
begin
  ...
  insert into public.variations
    (entry_id, description, directed_by, directed_at, vr_ref, estimated_cost,
     crew, photo_urls, source_quote, confidence)
  select p_entry_id, x.description, x.directed_by, x.directed_at, x.vr_ref, x.estimated_cost,
         nullif(x.crew, '{}'), coalesce(x.photo_urls, '{}'), x.source_quote, x.confidence
    from jsonb_to_recordset(coalesce(p_payload -> 'variations', '[]'::jsonb)) as x(
      ...
      estimated_cost numeric, crew text[], photo_urls text[], source_quote text,
