-- ============================================================================
-- Variations: the hours worked on it, on the day.
--
-- The owner: "add the hours worked on the variation within the days, rather
-- than the register." A variation's hours are a fact about a day — how long
-- the crew was on it — so they are recorded on the day's row, and the
-- register adds the days up. Nothing is typed on the register that a day
-- could have said.
--
-- `hours` is conditional in the hash, like crew and the register number:
-- absent when null, so every entry signed before today still verifies.
-- Never invented: unstated is null, and the docket prints a dash.
-- ============================================================================

alter table public.variations add column if not exists hours numeric
